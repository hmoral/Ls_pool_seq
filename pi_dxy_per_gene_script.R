...
# adapted by Hernan E. Morales for:
...
if("--help" %in% args) {
  cat("
      R script to calcuate pi and dxy per gene from sync files and a gff file
      
      Arguments:
      --arg1= sync file input (ending with .sync or .sync.gz if compressed)
      Note that this requires a genewise-sync file from popoolation2 using the create-genewise-sync.pl script
      ...
      --arg7= GFF file
      --arg8= minimum read count (default = 2)
...
# Load GTF file for coordinates
GTF<-fread(GFF_file,h=F,sep="\t")
GTF$gene<-str_split_fixed(str_split_fixed(GTF$V9,";",Inf)[,3],'"',Inf)[,2]
...
#syncFile = fread(cmd = paste("zcat",syncFile_input,"| head -40000"), sep = "\t")
syncFile = fread(syncFile_input, sep = "\t")
# add header
NoPops<-ncol(syncFile)-3
#syncFile[1:5,1:5]
names(syncFile)<-c("gene","position","ref",paste("pop",1:NoPops,sep=""))
print(paste("No. of pops in sync file = ",NoPops))
print(paste("pops selected = ",popA,"--",popB))
# generate gen list
geneList<-unique(syncFile$gene)
# generate progress bar
z<-0
imax<-length(geneList)
pb <- txtProgressBar(min = 0, max = imax, style = 3)
# loop over sync file, generate windows and calculate metrics
Pi_results<-data.table()
for(locus in geneList){
  z<-z+1
  GTF_gene<-GTF[gene == locus]
  CONTIG<-unique(GTF_gene$V1)
  START=min(c(GTF_gene$V4,GTF_gene$V5));END=max(c(GTF_gene$V4,GTF_gene$V5))
  if(length(CONTIG)>1){print(paste("Gene",locus,"relates to more than one contig!"))
    break}
  syncFile_gene = syncFile[gene == locus]
  #syncFile_gene[1:5,1:5]
  pop_data = data.frame(syncFile_gene[,4:ncol(syncFile_gene)])
  #pop_data = head(pop_data, 10000)
  slice = length(rownames(pop_data))
  #print(paste("Gene",locus,"sites = ",m))
  launch_function= pi_dxy_function(popA,popB)
  Pi_results_x<-data.table(gene=locus,contig=CONTIG,start=START,end=END,length=(END-START)+1,
